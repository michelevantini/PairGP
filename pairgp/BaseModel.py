...
class BaseModel(DiffConditionModel):
    ...
    def fit(
        ...
        self.partitions_ranking = pd.DataFrame(
            partition_models, columns=["p", "score"]
    ...
    def gp_base_fit(
        ...
        gp, score = self.gp_unit(X, y, kernel, normalizer=True)
    ...
    def generate_base_kernel(self):
        ...
        """
        kernel = GPy.kern.RBF(input_dim=1, useGPU=self.use_gpu, name="f")
    ...
    def plot(self, title=None):
        ...
        for i, subset in enumerate(self.partition):
            ...
            lower_interval, upper_interval = self.models[i].predict_quantiles(
                time_predN
            )
            ...
            plt.plot(time_pred, upper_interval, color=self.colors[i], linestyle="--")
            plt.plot(time_pred, lower_interval, color=self.colors[i], linestyle="--")
            plt.fill_between(
                time_pred,
                lower_interval,
                upper_interval,
            ...
            # print(time_pred)
            # print(y_pred_mean_f)
            plt.plot(time_pred, y_pred_mean_f, self.colors[i], label=label)
